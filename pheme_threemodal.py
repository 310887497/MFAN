...
os.environ["CUDA_VISIBLE_DEVICES"] = "0"  # 指定编号使用显卡
...
class Signed_GAT(nn.Module):  # 提取社交图特征
...
class NeuralNetwork(nn.Module):
    ...
    def mfan(self, x_tid, x_text, y, loss, i, total, params, pgd_word):
        ...
        # distance loss
        loss_mse = nn.MSELoss()
        loss_dis = loss_mse(dist_og[0], dist_og[1]) + loss_mse(dist_og[0], dist_og[2])
    ...
    def fit(self, X_train_tid, X_train, y_train,
        ...
        self.optimizer = torch.optim.Adam(self.parameters(), lr=1e-3, weight_decay=2e-3)  # 定义优化器
        ...
        pgd_word = PGD(self, emb_name='text_embedding.weight', epsilon=4, alpha=1.8)
        ...
        for epoch in range(self.config['epochs']):
            ...
            for i, data in enumerate(dataloader):
                ...
                batch_x_tid, batch_x_text, batch_y = (item.cuda(device=self.device) for item in data)  # 文本
    ...
    def predict(self, X_test_tid, X_test):  # 测试代码
        if torch.cuda.is_available():
            self.cuda()
        self.eval()
        y_pred = []
        ...
        for i, data in enumerate(dataloader):
            with torch.no_grad():
                ...
                #
                # probs = np.exp(logits.cpu().numpy()) / np.sum(np.exp(logits.cpu().numpy()), axis=1, keepdims=True)
                # preds = np.argmax(probs, axis=1)
                # class_0_probs = probs[:, 0]
                # plt.scatter(range(len(class_0_probs)), class_0_probs, c=preds, cmap='coolwarm')
                # plt.colorbar()
                #
                # # 添加坐标轴标题和图表标题
                # plt.xlabel('Sample index')
                # plt.ylabel('Probability of non-rumor')
...
class resnet50:
    def __init__(self):
        ...
        self.model.classifier = nn.Sequential(*list(model.classifier.children())[:-1])
        self.model.fc = nn.Linear(1000, 300).cuda()  # 修改resnet最后的全连接层
        torch.nn.init.eye_(self.model.fc.weight)  # 用单位矩阵填充
    ...
    def forward(self, xtid):
        ...
        for newid in xtid.cpu().numpy():
            ...
            imgpath = self.path + imgnum + '.jpg'  # 获得各个图片路径,如0.jpg,1.jpg
...
class VGG16:
    def __init__(self):
        ...
        self.model.fc = nn.Linear(1000, 300).cuda()
    ...
    def forward(self, xtid):
        ...
        batch_img = torch.cat(img_list, dim=0).cuda()
        img_output = self.model(batch_img)
        img_output = self.model.fc(img_output)
...
class CLIP:
    def __init__(self):
        ...
        self.fc = torch.nn.Linear(512, 300).cuda()  # 修改CLIP最后的全连接层
    ...
    def forward(self, xtid):
        ...
        for newid in xtid.cpu().numpy():
            ...
            img_list.append(im)
        batch_img = torch.cat(img_list, dim=0).cuda()
        image_features = self.model.encode_image(batch_img)
        image_features = self.dropout(self.relu(self.fc(image_features)))
...
class MFAN(NeuralNetwork):
    def __init__(self, config, adj, original_adj):
        ...
        self.mh_attention = TransformerBlock(input_size=300, n_heads=8, attn_dropout=0)  # 多头注意力机制
        ...
        self.image_embedding = CLIP()
        # self.textEncoder, self.preprocess = clip.load("ViT-B/32")
        # self.fc = torch.nn.Sequential(
        #     torch.nn.Linear(512, 300),
        #     torch.nn.ReLU(),
        #     torch.nn.Dropout(0.5)).cuda()  # 修改CLIP最后的全连接层
        # model_dict = torch.load("../clip_rumor_3.pth")
        # self.fc.load_state_dict(model_dict)
    ...
    def forward(self, X_tid, X_text):
        ...
        sim_matrix = torch.zeros((bsz, 3, 3)).to(device)
        for i in range(bsz):
            sim_matrix[i][0][1] = sim_matrix[i][1][0] = F.kl_div(F.log_softmax(self_att_t.view(bsz, 300)[i], dim=-1),
                                                                 F.softmax(self_att_g.view(bsz, 300)[i], dim=-1),
                                                                 reduction="batchmean")
            sim_matrix[i][0][2] = sim_matrix[i][2][0] = F.kl_div(F.log_softmax(self_att_t.view(bsz, 300)[i], dim=-1),
                                                                 F.softmax(self_att_i.view(bsz, 300)[i], dim=-1),
                                                                 reduction="batchmean")
            sim_matrix[i][1][2] = sim_matrix[i][2][1] = F.kl_div(F.log_softmax(self_att_g.view(bsz, 300)[i], dim=-1),
                                                                 F.softmax(self_att_i.view(bsz, 300)[i], dim=-1),
                                                                 reduction="batchmean")
        sim_matrix = F.softmax(sim_matrix, dim=-1)
        text_weight, graph_weight, image_weight = sim_matrix[:, 0, 1], sim_matrix[:, 0, 2], sim_matrix[:, 1, 2]
        # 加权平均处理
        text_feature_align = text_feature * text_weight.view(-1, 1) + rembedding * graph_weight.view(-1, 1) + \
                             iembedding * image_weight.view(-1, 1)
        align_text = self.alignfc_t(text_feature_align)

        # align_text = self.alignfc_t(text_enhanced)
        align_rembedding = self.alignfc_g(self_att_g).view(bsz, 300)
        align_iembedding = self.alignfc_i(self_att_i).view(bsz, 300)
        dist = [align_text, align_rembedding, align_iembedding]
        ...
        d1 = self.dropout(a1)
        output = self.fc2(d1)  # [batch_size, 2]
...
def load_dataset():
    ...
    with open(pre + '/new_id_dic.json', 'r', encoding='gb18030', errors='ignore') as f:
        newid2mid = json.load(f)
        newid2mid = dict(zip(newid2mid.values(), newid2mid.keys()))
    config['newid2mid'] = newid2mid
    content_path = os.path.dirname(os.getcwd()) + '/dataset/pheme/'
    with open(content_path + '/content.csv', 'r', encoding='gb18030', errors='ignore') as f:
        reader = csv.reader(f)
    ...
    return X_train_tid, X_train, y_train, \
           X_dev_tid, X_dev, y_dev, \
           X_test_tid, X_test, y_test, adj, newid2mid
...
def train_and_test(model):
    ...
    X_dev_tid, X_dev, y_dev, \
    X_test_tid, X_test, y_test, adj, newid2mid = load_dataset()
